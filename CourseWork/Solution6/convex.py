...
for t in range(len(X_test)):
    ...
    predicted_class.append(1+dist.index(min(dist)))
    # print("predicted = ", 1+dist.index(min(dist)), " and Truth is", int(Y_test[t]))
    if 1+dist.index(min(dist)) == int(Y_test[t]):
        correct += 1
...
# for test in X_test:
#     for i in range(10):
#         d = np.linalg.norm(test- mean_unseen[i])
#         dist.append(d)
#     predicted_class.append(dist.index(min(dist))+1)
#
# for i in range(len(Y_test)):
#     if predicted_class[i]== int(Y_test[0]):
#         correct += 1
#
# print(correct)